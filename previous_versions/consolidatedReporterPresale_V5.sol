...
contract ReporterTokenPresale is Ownable {
  ...
  uint256 public hardcap = 1 * 50 * (10**6) * (10**decimals);

  // maximum amount of tokens
  uint256 public tokenCreationCap = 1 * 30 * (10**6) * (10**decimals);
   // tokenCreationCap = 24 * 10**6 * 10**18; if this is in wei
  ...
  // default limit to tokens what the users can buy
  ...
  modifier validPurchase {
    ...
    require(tokenCreationCap > tokenRaised); // check we are not over the tokenCreationCAp
    _;
  }
  ...
  function hasEnded() public constant returns (bool) {
    ...
    bool capReached = tokenRaised >= tokenCreationCap; // if we reach the tokenCreationCap 
  ...
  function buyTokens(address beneficiary) payable validPurchase onlyAuthorised {
    ...
    require (token.balanceOf(msg.sender) + tokens <= maxTokens);
