...
contract ReporterToken is MintableToken, Pausable {
  string public name = "Reporter Community NEWS token";
...
contract ReporterTokenSale is Ownable, Pausable {
  ...
  // address to hold reserve tokens
  ...
  uint256 public constant maxContribution = 1 * (10 ** 6) * 10**decimals;

  // maximum amount of created tokens
  uint256 public maxTokens = 60 * (10**6) * (10**decimals);

  // maximum amount of tokens for sale now
  uint256 public tokensForSale = 18 * (10**6) * (10**decimals);  // 24 Million Tokens for SALE
  ...
  function ReporterTokenSale() {
    ...
    reserve = 0x1010101010101010101010; // get this right
  ...
  function hasEnded() public constant returns (bool) {
    ...
    function requireAuthorisation(bool state) onlyCSorOwner {
        freeForAll = !state;
    }

  // low level token purchase function
  function buyTokens(address beneficiary) payable onlyAuthorised  {
    ...
    require(msg.value >= minContribution);
    require (token.balanceOf(msg.sender) + tokens <= maxContribution); 
